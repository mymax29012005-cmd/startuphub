...
const dictionaries: Record<Lang, any> = {
  ru: {
    ...
    analyzer: {
      ...
      fields: {
        ...
        monthlyChurnPct: "Отток (churn) в месяц",
        ...
        viralCoefficient: "Вирусный коэффициент (K)",
      ...
      fieldHelp: {
        ...
        newRevenueMonthly:
          "Сколько новой выручки вы добавили за последний месяц (новые подписки, апсейлы, новые продажи). Нужна для burn‑мультипликатора и magic number.",
        salesMarketingSpend:
      ...
      tipText: {
        ...
        finance: "Запас денег (сколько месяцев проживёте) — критичный фактор риска.",
      },
  ...
  en: {
    ...
    analyzer: {
      ...
      steps: { market: "Рынок", traction: "Тракция", unit: "Юнит-экономика", finance: "Финансы и риски", report: "Отчёт" },
      section: {
        market: "Рынок и позиционирование",
        traction: "Тракция и команда",
        unit: "Юнит-экономика",
        finance: "Финансы и риск-профиль",
      },
      help: {
        market: "Оцените валидацию рынка и конкуренцию. Чем выше валидация — тем ниже риск.",
        traction: "Оцените тракцию и силу команды.",
        unit: "Заполните маржу, отток и ключевые параметры юнит-экономики.",
        finance: "Укажите burn, денежный запас и риск-профиль.",
      },
      fields: {
        marketValidation: "Валидация рынка",
        competition: "Интенсивность конкуренции",
        moatStrength: "Сила moаt",
        tractionScore: "Индекс тракции",
        teamStrength: "Сила команды",
        grossMargin: "Валовая маржа",
        monthlyChurnPct: "Отток (churn) в месяц",
        growthMonthly: "Рост в месяц",
        ...
        cac: "CAC (₽)",
        paybackMonths: "Окупаемость, мес",
        recurringShare: "Доля повторной выручки",
        burnMonthly: "Сжигание (burn) в месяц (₽)",
        cashOnHand: "Денежный запас (₽)",
        dau: "DAU",
        mau: "MAU",
        retentionD30: "Удержание D30 (%)",
        ...
        foundersFullTime: "Фаундер(ы) full‑time?",
        tam: "TAM (market size, ₽)",
        tamGrowthPct: "Рост TAM (%/год)",
        competitionDensity: "Плотность конкуренции (%)",
        regulatory: "Регуляторный риск",
        tech: "Технический риск",
      },

      fieldHelp: {
        marketValidation:
          "Насколько рынок подтверждён продажами/пилотами. Чем выше — тем ниже риск.",
        competition:
          "Насколько сильны конкуренты и сложно ли входить. Чем выше — тем больше риск.",
        moatStrength:
          "Moat — устойчивое преимущество (бренд, сеть, уникальная технология, cost advantage), которое сложно скопировать.",
        tractionScore:
          "Как быстро продукт доказывает спрос (рост/удержание/вовлечение). Чем выше — тем лучше.",
        teamStrength:
          "Сила команды: опыт, скорость обучения, способность исполнять план.",
        grossMargin:
          "Валовая маржа показывает, сколько остаётся после прямых затрат. Чем выше — тем устойчивее.",
        monthlyChurnPct:
          "Отток (churn) — доля клиентов, которые уходят каждый месяц. Ниже отток — выше LTV.",
        newRevenueMonthly:
          "Сколько новой выручки вы добавили за последний месяц (новые подписки, апсейлы, новые продажи). Нужна для burn‑мультипликатора и magic number.",
        salesMarketingSpend:
          "Сколько вы тратите в месяц на привлечение (реклама, маркетинг, продажи). Нужна для magic number.",
        arpu:
          "ARPU — средняя выручка на активного пользователя в месяц.",
        cac:
          "CAC — стоимость привлечения одного клиента/пользователя.",
        paybackMonths:
          "Сколько месяцев нужно, чтобы окупить CAC за счёт маржинальной прибыли.",
        burnMonthly:
          "Burn в месяц: сколько денег вы тратите (команда, маркетинг, инфраструктура).",
        cashOnHand:
          "Денежный запас: сколько денег у вас есть, чтобы дойти до следующих вех.",
        dau:
          "DAU — активные пользователи в день. Обычно берут из аналитики (Firebase/Amplitude и т.п.).",
        mau:
          "MAU — активные пользователи в месяц. Вместе с DAU даёт stickiness (DAU/MAU).",
        retentionD30:
          "Доля пользователей, которые возвращаются на 30-й день. Когортная метрика; чем выше — тем ближе PMF.",
        repeatPurchaseRate:
          "Доля клиентов, которые покупают повторно/продлевают. Для подписок — данные по продлениям.",
        organicGrowthPct:
          "Рост из органики (SEO/сарафан/PR), а не из платной рекламы.",
        viralCoefficient:
          "K‑factor: сколько новых пользователей приводит один активный пользователь (рефералка/инвайты).",
        releasesPerMonth:
          "Сколько релизов/обновлений вы выпускаете в месяц. Прокси скорости исполнения.",
        teamSize:
          "Текущий размер команды (включая фаундеров).",
        foundersFullTime:
          "Работают ли фаундеры над проектом full‑time.",
        tam:
          "TAM — общий размер рынка, куда вы потенциально можете продавать (в год, ₽). Обычно из исследований.",
        tamGrowthPct:
          "Годовой темп роста рынка (%).",
        competitionDensity:
          "Насколько рынок «перегрет» конкурентами: 0% почти нет, 100% очень тесно.",
        regulatory:
          "Регуляторный риск: вероятность юридических/комплаенс‑проблем.",
        tech:
          "Технический риск: сложность поддержки продукта и надёжность технологического стека.",
        growthMonthly:
          "Как быстро растёт ключевая метрика каждый месяц (пользователи/выручка).",
        recurringShare:
          "Доля повторной выручки (подписки/контракты/регулярные платежи).",
      },
      levels: { low: "Низкий", medium: "Средний", high: "Высокий" },
      tip: { market: "Как оценивать рынок", traction: "Как оценивать тракцию", unit: "Как оценивать юнит‑экономику", finance: "Как оценивать финансы" },
      tipText: {
        market: "Если есть продажи, пилоты и повторяемый спрос — валидация выше.",
        traction: "Оцените, насколько быстро продукт доказывает спрос (рост/удержание/вовлечение).",
        unit: "Маржа и отток напрямую влияют на LTV и устойчивость роста.",
        finance: "Запас денег (сколько месяцев проживёте) — критичный фактор риска.",
      },
      actions: {
        back: "Назад",
        ...
        createIdeaWithAnalysis: "Создать идею с этим анализом",
        saving: "Сохраняем…",
        loginToSave: "Войдите, чтобы сохранить анализ.",
      },
      ...
      report: {
        ...
        runway: "Запас денег",
        burn: "Сжигание (burn)",
        expectedValue: "Ожидаемая ценность (E[V])",
        expectedValueHint: "С учётом вероятности успеха",
        unitEconomicsTitle: "Юнит‑экономика",
        ltv: "LTV",
        payback: "Окупаемость",
        valuationTitle: "Диапазон оценки",
        valuationText1: "Диапазон считается детерминированно (мультипликаторы + упрощённый DCF), без ИИ.",
        valuationLow: "Нижняя граница",
        valuationHigh: "Верхняя граница",
        financialTitle: "Финансовые метрики",
        arr: "ARR (оценка повторяемой выручки)",
        multiple: "Мультипликатор",
        breakeven: "Безубыточность (выручка/мес)",
        npv: "NPV (5 years)",
        ...
        riskNote: "Чем выше значение — тем выше шанс проблем.",
      ...
      historyTitle: "История анализов",
      historyLogin: "Войдите, чтобы видеть сохранённые отчёты.",
      historySubtitle: "Сохранённые отчёты привязаны к аккаунту.",

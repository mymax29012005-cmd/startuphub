  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const data = (await r.json()) as InvestorItem[];
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10">
      <div className="flex items-end justify-between gap-4 mb-5">
        <div>
          <div className="text-[rgba(234,240,255,0.72)] text-sm">{t("nav.investors")}</div>
          <h1 className="text-2xl font-semibold">{t("nav.investors")}</h1>
        </div>
        <a href="/add-investor" className="text-[var(--accent)] hover:text-white text-sm font-medium">
          {t("pages.addInvestor")}
        </a>
      </div>
      ...
      <Card className="p-6">
        {loading ? (
          ...
          <div className="text-[rgba(234,240,255,0.72)]">{t("pages.emptyInvestors")}</div>
        ) : (
          <div className="grid grid-cols-1 gap-4">
            {items.map((it) => (
              ...
              >
                <Link href={`/investors/${it.id}`} className="flex-1 min-w-0 block">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <div className="mt-3 font-semibold text-white">
                        Сумма: {Number(it.amount).toLocaleString("ru-RU")} ₽
                      </div>

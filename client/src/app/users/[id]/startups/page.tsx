...
export default function UserStartupsPage({ params }: { params: Promise<{ id: string }> }) {
  const { t } = useI18n();
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        const s = await fetch(`/api/v1/startups?ownerId=${id}`, { cache: "no-store" });
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10">
      <div className="mb-5 flex items-center justify-between gap-4">
        <Link href={`/users/${id}`} className="text-[var(--accent)] hover:text-white text-sm font-medium">
          ← Профиль
        </Link>
      </div>
